...
function UpdateMoney()
	...
	if where == Constants.Position.TITANBAR then
		...
		if diff < 0 then
			diff = math.abs(diff); bIncome = false;
		end
		...
		if SSS < 0 then
			SSS = math.abs(SSS); bSumSSS = false;
		end
		...
		if STS < 0 then
			STS = math.abs(STS); bSumSTS = false;
		end
...
function InitializeMoneyInfos()
	...
	MI["GIcon"] = CreateControlIcon(MI["GCtr"], Constants.MONEY_ICON_WIDTH, Constants.MONEY_ICON_HEIGHT,
		resources.MoneyIcon.Gold)
	...
	MI["SIcon"] = CreateControlIcon(MI["SCtr"], Constants.MONEY_ICON_WIDTH, Constants.MONEY_ICON_HEIGHT,
		resources.MoneyIcon.Silver)
	...
	MI["CIcon"] = CreateControlIcon(MI["CCtr"], Constants.MONEY_ICON_WIDTH, Constants.MONEY_ICON_HEIGHT,
		resources.MoneyIcon.Copper)
