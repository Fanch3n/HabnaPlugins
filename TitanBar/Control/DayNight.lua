...
function UpdateDayNight()
	...
	if dnData.next ~= false then --Show next day & night time
		if totalseconds >= 60 then
			NDNTime = cdminutes .. " min: " .. ntimer;
		else
			NDNTime = totalseconds .. " sec: " .. ntimer;
		end
	...
	if sDay == "day" then
		_G.ControlData.DN.controls["Icon"]:SetBackground(resources.Sun);
		-- Sun in-game icon (0x4101f898 or 0x4101f89b)
	else
		_G.ControlData.DN.controls["Icon"]:SetBackground(resources.Moon);
	end                                                                           -- Moon in-game icon
